...
public class CacheManager {
    ...
    private DiskLruCache_ mDiskLruCache;
    ...
    private CacheManager() {
        File diskCacheDir = getDiskCacheDir(App.activity, CACHE_DIR);
        ...
        if (diskCacheDir.getUsableSpace() > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = DiskLruCache_.open(diskCacheDir,
                        getAppVersion(App.activity), 1/*一个key对应多少个文件*/, DISK_CACHE_SIZE);
    ...
    public void putCache(String key, String value) {
        ...
        try {
            DiskLruCache_.Editor editor = mDiskLruCache.edit(encryptMD5(key));
    ...
    public String getCache(String key) {
        ...
        try {
            DiskLruCache_.Snapshot snapshot = mDiskLruCache.get(encryptMD5(key));
            if (snapshot != null) {
